...
import { LayoutDashboard, Sparkles, Award } from "lucide-react";
...
const InterviewerPage = () => {
  ...
  // Simple data processing with candidate filtering
  ...
  const processedData = useMemo(() => {
    ...
    candidates.forEach((candidate) => {
      ...
      if (email && candidate.email !== decodeURIComponent(email)) {
        return;
      }
  ...
  const stats = useMemo(() => {
    ...
    // If viewing a specific candidate, filter data for that candidate only
    const targetCandidates = email
      ? candidates.filter(
          (candidate) => candidate.email === decodeURIComponent(email)
        )
      : candidates;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Breadcrumb
            items={[
              ...
              ...(email && processedData.length > 0
                ? [
                    {
                      title: (
                        <span className="inline-flex items-center">
                          {processedData[0]?.candidateName
                            ? processedData[0].candidateName
                                .toLowerCase()
                                .split(" ")
                                .map(
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <Title level={2} className="!mb-0 !text-2xl !font-bold">
                    {email && processedData.length > 0
                      ? `${
                          processedData[0]?.candidateName || "Candidate"
                        } 's profile`
                      : "Interview Dashboard"}
                  </Title>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-violet-50 rounded-lg border border-violet-100">
                      ...
                      <Text className="text-sm font-semibold text-violet-700">
                        {stats.total || 0}{" "}
                        {stats.total === 1 ? "Interview" : "Interviews"}
                      </Text>
                    </div>
                    <Text className="text-gray-500 text-sm">
                      {email ? "completed by this candidate" : "total"}
                    </Text>
                  </div>
                </div>
              </div>
              ...
              {email && (
                <div className="text-right">
                  ...
                  <div className="text-2xl font-bold text-violet-600">
                    {processedData[0]?.score || "N/A"}
                    {processedData[0]?.score && "%"}
                  </div>
                </div>
              )}
            </div>
